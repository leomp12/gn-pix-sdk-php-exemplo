...
$body = json_encode([
  ...
  "devedor" => [
    ...
    //"cnpj" => "12345678000123",
    "nome" => "Nome Aqui" // [opcional] Nome do usuário pagador. string (Nome) ≤ 200 characters
  ],
  "valor" => [
    "original" => "1.49" // [obrigatório/opcional] Valor original da cobrança.string \d{1,10}.\d{2} Obs: Na cobrança para QR Code dinâmico é obrigatório um valor, para estático não
  ],
...
if ($tipo === "dinamico") { // Somente será gerado o payload para o QR Code dinâmico
  $dadosPix = emitePix($pix_url_cob, $arq_certificado, $body, $tokenType, $accessToken);
